...
public class ServersControllerTest {
    ...
    public void testGetList() {
        ...
        try {
            mockMvc.perform(get("/api/servers")).andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(1)))
    ...
    public void testCreate() throws Exception {
        ...
        mockMvc.perform(
                post("/api/servers").contentType("application/json").content(
                        TestUtil.convertObjectToJsonBytes(serverDto))).andExpect(status().isOk())
